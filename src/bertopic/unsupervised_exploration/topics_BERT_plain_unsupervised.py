"""UNFILTERED: Unsupervised topic exploration of GreatSchools reviews (BERTopic).

Fits on the full review corpus, not restricted to schools with an NCES
match / friending-bias score -- see topics_BERT_plain_unsupervised_filtered.py
for the counterpart fit only on the nces-filtered corpus.

Before hand-specifying which "concepts" to look for in reviews -- an approach
this project ultimately pursued via GABRIEL instead, not as a follow-on
numbered stage -- this script asks the opposite question: what is actually
*in* the reviews? Letting an unsupervised model surface the recurring themes
on its own avoids the trap of only measuring what we already thought to
look for.
...
per-review file (gs_reviews_sorted_by_day_posted.csv) and keep each review's
`universal-id` so topics can be aggregated back to the school level downstream
(topics_distro_BERT*.py, topics_BERT_classbased*.py).
...
We FIT on the full corpus (all ~681K reviews). Embedding + UMAP + HDBSCAN over
that many reviews is heavy, so expect a long run (GPU/MPS strongly preferred).
...
MIN_TOPIC_SIZE=200, not the original 50: at 50, HDBSCAN produced 952 topics
with a long fat tail, and inspecting the low end showed most of that tail was
not real content themes but single-school clusters formed purely because a
review repeats its own school's distinctive name (e.g. "st theresa, theresa,
teresa"; "walden, walden school, walden green") -- reviews that share a rare
proper noun, not a topic. A downstream regression using these topics as
features got essentially zero importance from that entire tail. Raising the
threshold pushes those small, non-generalizable clusters into noise (or merges
them into a real topic), leaving fewer, larger, more genuinely thematic
topics. All outputs embed MIN_TOPIC_SIZE in their name so this run is never
confused with the min_cluster=50 one still on disk.

Outputs:
    outputs/models/bertopic/bertopic_model_{MIN_TOPIC_SIZE}_min_cluster/ (the fitted model)
    outputs/bertopic/unsupervised_exploration/assignments/topic_assignments_{MIN_TOPIC_SIZE}_min_cluster.csv
        (universal-id, review id, topic)
    outputs/bertopic/unsupervised_exploration/summary/topic_summary_{MIN_TOPIC_SIZE}_min_cluster.csv
        (topic_id, size, share, keywords, mean_words)
    outputs/bertopic/unsupervised_exploration/representative_docs/topic_representative_docs_{MIN_TOPIC_SIZE}_min_cluster.csv
        (top-N reviews per topic, top TOP_N_TOPICS topics by size)

Run from the project root as a module:
    python -m src.bertopic.unsupervised_exploration.topics_BERT_plain_unsupervised
"""
...
model_dir = f"outputs/models/bertopic/bertopic_model_{MIN_TOPIC_SIZE}_min_cluster"
table_dir = "outputs/bertopic/unsupervised_exploration"
assignments_path = f"{table_dir}/assignments/topic_assignments_{MIN_TOPIC_SIZE}_min_cluster.csv"
summary_path = f"{table_dir}/summary/topic_summary_{MIN_TOPIC_SIZE}_min_cluster.csv"
repr_docs_path = f"{table_dir}/representative_docs/topic_representative_docs_{MIN_TOPIC_SIZE}_min_cluster.csv"


def load_reviews(path=reviews_path, random_state=RANDOM_STATE):
    """Read the per-review file and drop empty reviews. Fits on the full corpus.
    Returns school id, NCES id, review id, date, text, words."""
    df = pd.read_csv(
