"""CONCAT, FILTERED: Topic-mixture GBM model for friending bias -- nces-filtered corpus.

IMPORTANT: this script's counterpart, topic_regression_per_review.py, is NOT
a clean single-variable comparison against this one. The two scripts
simultaneously differ on three axes at once -- (1) filtered vs. unfiltered
corpus, (2) concat vs. per-review topic aggregation, and (3) MIN_CLUSTER_SIZE
(50 vs. 200) -- so any R^2 difference between them reflects all three, not
filtering alone. Don't read a headline-number gap between the two as evidence
about filtering in isolation.
...
MIN_CLUSTER_SIZE=50 on the nces-filtered review corpus
(topics_BERT_plain_unsupervised_filtered.py), scored directly on each
school's concatenated-review blob by topics_distro_BERT_concat_filtered.py
(outputs/bertopic/unsupervised_exploration/distributions/
topic_distributions_{MIN_CLUSTER_SIZE}_min_cluster_by_school_concat_filtered.csv).
This isolates how much of friending bias the CONTENT
of what people say about a school explains, within the schools that already
have both an nces_id and a friending-bias score.

Unlike topic_regression_per_review.py's MIN_CLUSTER_SIZE=200 run, which fit
BERTopic on the full ~618K-review corpus and hand-picked a 235-topic subset,
this run fits BERTopic only on the 69,238 nces-filtered reviews and uses ALL
85 real topics that HDBSCAN found at that threshold (no cutoff needed -- see
topics_distro_BERT_concat_filtered.py). Every output name embeds
MIN_CLUSTER_SIZE, the topic count, and "concat_filtered" so this run is
never confused with the per-review/unfiltered run.

Keyed on nces_id: the topic-distribution table is already restricted to the
11,859 schools with a matching row in gs_demos_with_social_capital.csv, so
the join here is on nces_id to match it (same as topic_regression_per_review.py).

Outputs:  outputs/models/topic_regression/topic_gbm_top{N}_min_cluster{MIN_CLUSTER_SIZE}_concat_filtered.pkl
          outputs/bertopic/topic_regression/topic_gbm_metrics_top{N}_min_cluster{MIN_CLUSTER_SIZE}_concat_filtered.csv
          outputs/bertopic/topic_regression/topic_gbm_importances_top{N}_min_cluster{MIN_CLUSTER_SIZE}_concat_filtered.csv
"""
