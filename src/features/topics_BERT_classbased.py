"""Stage 2c — topics per class (BERTopic), splitting the flat topics from
topics_BERT_plain_unsupervised.py by friending-bias group.
...
same circle/square convention the GABRIEL pipeline uses (src/features/pairs.py):
...
were matched, see reports/stage0_data_summary.md) get a bookkeeping class of
...
pipeline (src/features/topics_gabriel.py): it asks a version of the same
...
(outputs/models/bertopic_model/) when present, same fast-path pattern as
topics_BERT_hierarchical.py — no re-embedding/re-clustering needed.

Outputs:
    outputs/tables/topics_per_class.csv     (topic x class rows: words, frequency)
    outputs/figures/topics_per_class.png    (grouped bar chart, raw frequency)
    outputs/figures/topics_per_class_normalized.png
...
table_path = "outputs/tables/topics_per_class.csv"
fig_path = "outputs/figures/topics_per_class.png"
fig_path_normalized = "outputs/figures/topics_per_class_normalized.png"
...
def plot_topics_per_class(model, known, top_n_topics=TOP_N_TOPICS,
                           ...
                           title=None):
    """Grouped horizontal bar chart: for each of the top-N globally most
    frequent topics, one bar for low_fb and one for high_fb -- 2 * top_n_topics
    bars total, all visible at once.
    ...
    topic_names = {
        topic: (label[:40] + "..." if len(label) > 40 else label)
        for topic, label in model.topic_labels_.items()
    }
    ...
    for cls, color in [("low_fb", "#56B4E9"), ("high_fb", "#E69F00")]:
        ...
        fig.add_trace(go.Bar(
            y=sub["Label"], x=sub[value_col], name=cls,
            orientation="h", marker_color=color,
            hoverinfo="text",
            hovertext=[
                f"Topic {t}<br>Words ({cls}): {w}<br>{value_col}: {v:.3g}"
    ...
    fig.update_layout(
        barmode="group",
        title=title or f"Top {top_n_topics} topics by global frequency: low_fb vs high_fb",
...
def run():
    ...
    fig_norm = plot_topics_per_class(
        ...
        title=f"Top {TOP_N_TOPICS} topics by global frequency: low_fb vs high_fb "
