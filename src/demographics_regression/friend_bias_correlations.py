...
No filtered/unfiltered counterpart: this reads only
gs_demos_with_social_capital.csv (the full 13,958-school Atlas-matched
population) and never touches review text, so the filtered-vs-unfiltered
review-corpus distinction that applies to the BERTopic/GABRIEL scripts
doesn't apply here.
...
summary table is written alongside it.
"""
...
fig_dir = "outputs/demographics_regression"
...
def run_correlations(merged_path=merged_path, factors=FACTORS,
    ...
    table_path = f"{table_dir}/predictors_of_friending_bias_n_schools.csv"
