...
It's read straight from a school-concatenated reviews CSV (built by
src/data/concat_reviews.py or concat_reviews_nces_filtered.py from the
chronologically-sorted per-review file), so every stage of the project
concatenates reviews the same way: oldest-first, space-joined, no
reviewer-role prefix. This makes the reviews pure.

Corpus note: topics_gabriel.py's default now points this at the canonical
NCES-filtered concat file, matching the rest of the project -- but that
default choice barely matters, because ``build_fb_pairs`` does its own
filtering regardless of which review file is passed in: it drops every
school with no friending-bias score (``df_demos`` above) before inner-joining
that against the review file on ``universal-id``, so a school without a bias
score or without any reviews never enters a pair either way. This matters
because this file's pairing logic is adapted from Nabeel's original code,
which self-filtered the same way, and was deliberately not majorly
restructured here. It was verified empirically (2026-08-25, back when the
default was still the UNFILTERED gs_reviews_concat_by_school.csv) that this
self-filtering alone -- with no NCES-based filtering at all -- already
produces EXACTLY the same 11,859 schools used everywhere else in the pipeline
(the same nces_id set as
``data/processed/gs_reviews_concat_by_school_nces_filtered.csv``, zero
difference either direction). That equivalence holds because
``gs_demos_with_social_capital.csv`` already has a clean 1:1
universal-id<->nces_id mapping (deduplicated back in join_by_nces.py), so
joining on universal-id here was always safe, regardless of which review
file's name carries a "filtered" label.
"""
